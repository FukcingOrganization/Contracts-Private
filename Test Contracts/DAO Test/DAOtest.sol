...
contract DAOtest {
    ...
    function newProp (string memory _description, uint256 _lenght) public returns (bool) {        
        Proposal storage newProp = props[propCounter.current()];
    ...
    function propInfo (uint256 _propID) view public returns (propStatus, uint256, string memory, bool) {
        return (props[_propID].status , props[_propID].lenght, props[_propID].description, props[_propID].yayCount > props[_propID].nayCount);
    }
    function propResult (uint256 _propID) view public returns (bool) {       
        Proposal storage prop = props[_propID];
        require(prop.startTime + prop.lenght < block.timestamp, "The prop is stil ongoing");
